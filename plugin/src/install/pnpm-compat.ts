...
export type PnpmFailureCode =
  | "ignored-builds" | "peer-dependency" | "prepare-failed" | "lifecycle-failed"
  | "hoist-drift" | "release-age" | "host-peer"
  | "git-network" | "fetch-timeout" | "transient-network";
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    try {
      const event = JSON.parse(trimmed) as {
        code?: unknown; message?: unknown; err?: { code?: unknown; message?: unknown };
      };
...
export function classifyInstallFailure(result: InstallResult): InstallFailure | null {
  if (result.cancelled) return { code: "cancelled", message: "安装已取消。" };
  if (result.timedOut) return { code: "install-timeout", message: "安装超过总时限。请查看最后的执行日志，确认网络或构建前提后重试。" };
...
export async function withPnpmRecovery(
  ...
  if ((args[0] !== "add" && args[0] !== "remove")
    || !failure
    || !["git-network", "fetch-timeout", "transient-network"].includes(failure.code)) return first;
