...
function workspaceDocument(source: string): { document: Record<string, unknown>; approvals: Record<string, boolean>; duplicates: boolean } {
  interface ParsedNode { kind: string; result: unknown; children: ParsedNode[] }
  const stack: ParsedNode[] = [];
  const approvals: Record<string, boolean> = Object.create(null);
  let root: ParsedNode | undefined;
  ...
  const document: unknown = load(source, {
    ...
    listener(event, state) {
      if (event === "open") { stack.push({ kind: "", result: undefined, children: [] }); return; }
  ...
  function unwrap(node: ParsedNode): ParsedNode {
    while (node.children.length === 1 && node.children[0].result === node.result) node = node.children[0];
    return node;
  }
  function inspect(raw: ParsedNode, topLevel: boolean): void {
    const node = unwrap(raw);
    if (node.kind === "mapping") {
      const keys = new Set<string>();
      for (let index = 0; index < node.children.length; index += 2) {
        const key = node.children[index].result;
        if (typeof key !== "string") throw new Error("工作区配置需要字符串映射键");
        const allowedDuplicate = topLevel && key === "allowBuilds";
        if (keys.has(key)) {
          if (!allowedDuplicate) throw new Error(`工作区配置包含重复映射键：${key}`);
          duplicates = true;
        }
        keys.add(key);
        if (topLevel && key === "allowBuilds") {
          const value = node.children[index + 1]?.result;
          if (value !== null && !isMapping(value)) throw new Error("allowBuilds 必须是布尔值映射");
          for (const [name, decision] of Object.entries(value ?? {})) {
            if (typeof decision !== "boolean") throw new Error(`allowBuilds 中 ${name} 必须是 true 或 false`);
            approvals[name] = decision;
          }
        }
      }
    }
    for (const child of node.children) inspect(child, false);
  }
  if (root) inspect(root, true);
  const visiting = new Set<object>();
  function assertAcyclic(value: unknown): void {
    if (value === null || typeof value !== "object") return;
    if (visiting.has(value)) throw new Error("工作区配置包含递归 YAML 引用，无法安全合并");
    visiting.add(value);
    for (const child of Object.values(value)) assertAcyclic(child);
    visiting.delete(value);
  }
  assertAcyclic(document);
  if (document !== undefined && document !== null && !isMapping(document)) throw new Error("工作区配置必须是映射");
  const inherited = isMapping(document) ? document.allowBuilds : undefined;
  if (inherited !== undefined && inherited !== null) {
    if (!isMapping(inherited)) throw new Error("allowBuilds 必须是布尔值映射");
    for (const [name, decision] of Object.entries(inherited)) {
      if (typeof decision !== "boolean") throw new Error(`allowBuilds 中 ${name} 必须是 true 或 false`);
      if (!Object.hasOwn(approvals, name)) approvals[name] = decision;
    }
  }
  return { document: (document ?? {}) as Record<string, unknown>, approvals, duplicates };
}
...
export function allowPackageBuild(
  ...
  if (!duplicates && requested.every((key) => approvals[key] === true)) return false;
  for (const key of requested) approvals[key] = true;
  document.allowBuilds = { ...approvals };
  const next = dump(document, { lineWidth: -1, noRefs: true }).replace(/\n/g, source.includes("\r\n") ? "\r\n" : "\n");
  const roundTrip: unknown = load(next);
  if (!isDeepStrictEqual(roundTrip, document)) throw new Error("工作区配置序列化校验失败，已停止修改");
