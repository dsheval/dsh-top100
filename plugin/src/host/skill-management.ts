...
export async function withSkillMutationLock<T>(operation: () => Promise<T> | T, signal?: AbortSignal): Promise<T> {
  const previous = skillMutationTail;
  let release!: () => void;
  skillMutationTail = new Promise<void>((resolve) => { release = resolve; });
  await previous;
  try {
...
function assertDirectory(path: string, label: string): boolean {
  ...
  if (info?.isSymbolicLink()) throw new Error(`${label}是符号链接，已停止修改`);
  if (info && !info.isDirectory()) throw new Error(`${label}不是目录，已停止修改`);
  return info !== undefined;
}

export function assertGlobalSkillRoot(): void {
...
export function skillTarget(name: string): string {
  if (!SKILL_NAME_RE.test(name)) throw new Error("Skill 目录名无效");
...
export function skillContentManifest(root: string, ignoreGit = false): { digest: string; files: string[] } {
  if (!assertDirectory(root, "Skill 目录")) throw new Error("Skill 未安装");
  const files: string[] = [];
  const visit = (directory: string): void => {
    for (const entry of readdirSync(directory, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
      const path = join(directory, entry.name);
      const info = lstatSync(path);
      if (info.isSymbolicLink()) throw new Error(`Skill 包含符号链接，已停止修改：${entry.name}`);
      if (ignoreGit && entry.name === ".git") continue;
      if (info.isDirectory()) visit(path);
      else if (info.isFile()) files.push(relative(root, path).replaceAll("\\", "/"));
      else throw new Error(`Skill 包含非普通文件，已停止修改：${entry.name}`);
    }
  };
  visit(root);
...
export function inspectSkill(name: string, evidence?: SkillEvidence): SkillInspection {
...
export function backupSkill(name: string): SkillBackup {
  const inspection = inspectSkill(name);
  if (!inspection.installed) throw new Error("Skill 未安装");
  ...
  try {
    ...
    try { rmdirSync(container); } catch { /* preserve the original rename error */ }
    throw error;
  }
...
export function restoreSkillBackup(backup: SkillBackup): void {
  const target = skillTarget(backup.name);
  if (lstatSync(target, { throwIfNoEntry: false })) throw new Error("Skill 恢复目标已存在，原备份已保留");
  const root = join(skillHome(), "skill-backups");
  assertDirectory(root, "Skill 备份目录");
  const path = resolve(backup.backupPath);
  const parts = relative(root, path).split(sep);
  if (parts.length !== 2 || !parts[0]?.startsWith(`${backup.name}-`) || parts[0] === ".."
    || parts[1] !== backup.name || basename(path) !== backup.name) throw new Error("Skill 备份路径无效");
  if (!assertDirectory(dirname(path), "Skill 备份容器") || !assertDirectory(path, "Skill 备份")) throw new Error("Skill 备份不存在");
  skillContentManifest(path);
  renameSync(path, target);
  // Successful restoration must not be reported as failure over an empty container.
  try { rmdirSync(dirname(path)); } catch { /* harmless empty backup container */ }
}
