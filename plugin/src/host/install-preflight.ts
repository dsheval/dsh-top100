...
import { verifySkillSource, type VerifiedSkillSource } from "../install/skill-install.js";
import type {
  InstallPreflight,
  InstallProvenance,
  InstallRiskEvidence,
...
const approvals = new Map<string, ApprovedInstall>();

function removeExpiredApprovals(now = Date.now()): void {
  for (const [token, approval] of approvals) {
    if (approval.preflight.expiresAt <= now) approvals.delete(token);
  }
}

function bundleProvenance(value: VerifiedInstallTarget): InstallProvenance {
  return {
    source: value.source,
...
function bundleRisks(value: VerifiedInstallTarget): InstallRiskEvidence[] {
  const risks: InstallRiskEvidence[] = [];
