...
export interface SourceMigrationItem { name: string; from: string; version: string }
export interface SourceMigrationPreflight { approvalToken: string; items: SourceMigrationItem[]; expiresAt: number }
interface FileSnapshot { path: string; content: Buffer | null; hash: string | null; realPath: string | null; mode: number; writable: boolean }
interface MigrationApproval {
  profile: string; directory: string; preflight: SourceMigrationPreflight;
  snapshots: FileSnapshot[]; changes: Array<{ before: FileSnapshot; content: Buffer }>;
}
...
const pathExists = (path: string) => lstatSync(path, { throwIfNoEntry: false }) !== undefined;

function snapshot(path: string, writable = false, optional = false): FileSnapshot {
  let entry;
  try { entry = lstatSync(path); }
  catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw new Error("无法读取 Profile 元数据，已停止迁移");
    if (!optional) throw new Error(`缺少 ${path.endsWith("pnpm-lock.yaml") ? "pnpm-lock.yaml" : "必要的安装文件"}，无法安全固定当前版本`);
    return { path, content: null, hash: null, realPath: null, mode: 0o600, writable };
  }
  if (writable && entry.isSymbolicLink()) throw new Error("Profile 元数据使用符号链接，无法安全执行来源迁移");
  if (!statSync(path).isFile()) throw new Error("Profile 元数据不是普通文件，无法安全执行来源迁移");
  const content = readFileSync(path);
  return { path, content, hash: hash(content), realPath: realpathSync(path), mode: statSync(path).mode & 0o777, writable };
}

function assertSnapshots(snapshots: readonly FileSnapshot[]): void {
...
function parseJson(snapshot: FileSnapshot): Record<string, any> {
  try { const value: unknown = JSON.parse(snapshot.content!.toString("utf8")); if (record(value)) return value; } catch { /* Never quote raw local configuration. */ }
  throw new Error("Profile 或插件 package.json 损坏，无法安全迁移");
}

function parseYaml(snapshot: FileSnapshot): Record<string, any> {
  try {
    const value: unknown = load(snapshot.content!.toString("utf8"), { schema: JSON_SCHEMA });
    if (record(value)) return JSON.parse(JSON.stringify(value));
...
function removeExpired(): void {
  for (const [token, approval] of approvals) if (approval.preflight.expiresAt <= Date.now()) approvals.delete(token);
}

export function preflightSourceMigration(profile: string, explicitDir?: string): SourceMigrationPreflight {
  removeExpired();
  const directory = realpathSync(resolve(profileDir(profile, explicitDir)));
  const snapshots: FileSnapshot[] = [];
  const read = (path: string, writable = false, optional = false) => {
    const value = snapshot(path, writable, optional); snapshots.push(value); return value;
  };
  ...
  for (let ancestor = dirname(directory); ; ancestor = dirname(ancestor)) {
    if (read(join(ancestor, "pnpm-workspace.yaml"), false, true).content !== null) throw new Error("当前 Profile 属于外部 workspace，请先按原工作区方式固定依赖");
  ...
  for (const filename of [".pnpmfile.cjs", ".pnpmfile.mjs"]) {
    if (read(join(directory, filename), false, true).content !== null) throw new Error("Profile 使用自定义 pnpm hook，请先按原安装方式固定依赖");
  }
  if (workspaceFile.content !== null) {
    ...
    if (Object.keys(workspace).some((key) => !supported.has(key))
      || (workspace.packages !== undefined && (!Array.isArray(workspace.packages) || workspace.packages.some((path: unknown) => path !== ".")))) {
  ...
  const items: SourceMigrationItem[] = [];
  const evidence: InstallPreflight[] = [];
  for (const [name, from] of Object.entries(dependencies)) {
    if (npmPackageSpec(name)?.name !== name || typeof from !== "string" || !record(lockedDependencies[name])
      || lockedDependencies[name].specifier !== from) throw new Error("Profile dependencies 与锁文件安装来源不一致，请先修复安装");
    if (parseNpmSelector(from)?.kind !== "tag") continue;
    ...
    if (typeof integrity !== "string" || !integrity.trim()) throw new Error(`${name} 的锁文件缺少可核对的安装摘要，无法安全记录来源`);
  ...
  try { ledgerContent = renderBundleProvenance({ profile, profileDirectory: directory, dataUrl: "" }, evidence); }
  catch { throw new Error("已有安装来源台账损坏或无法安全读取，已停止迁移"); }
...
export function applySourceMigration(token: string, profile: string, explicitDir?: string): { items: SourceMigrationItem[]; migrated: number } {
  removeExpired();
  const approval = approvals.get(token);
  if (!approval) throw new Error("来源迁移确认已过期，请重新检查");
  ...
  const staged: Array<{ path: string; temporary: string; backup: string; before: FileSnapshot; expectedHash: string; hadOriginal: boolean; movedOriginal: boolean; published: boolean }> = [];
  const createdDirectories: string[] = [];
  try {
    for (const change of approval.changes) {
      const parent = dirname(change.before.path);
      if (!existsSync(parent)) { mkdirSync(parent); createdDirectories.push(parent); }
      if (lstatSync(parent).isSymbolicLink() || realpathSync(parent) !== parent) throw new Error("Profile 目录在迁移前发生变化，请重新检查");
    ...
    for (const entry of staged) {
      ...
      if (entry.hadOriginal) { renameSync(entry.path, entry.backup); entry.movedOriginal = true; }
      if (pathExists(entry.path)) throw new Error("迁移期间存在外部文件修改");
      renameSync(entry.temporary, entry.path); entry.published = true;
    }
    for (const entry of staged) {
      const current = snapshot(entry.path, true);
      if (current.hash !== entry.expectedHash || current.mode !== entry.before.mode) throw new Error("迁移期间存在外部文件修改");
    }
  } catch {
    let restored = true;
    for (const entry of [...staged].reverse()) {
      try {
        if (entry.movedOriginal) {
          ...
          if (ours || (!entry.published && !pathExists(entry.path))) renameSync(entry.backup, entry.path);
          else restored = false; // Keep both external changes and the original backup.
        } else if (entry.published) {
          const current = pathExists(entry.path) ? snapshot(entry.path, true) : null;
          if (current?.hash === entry.expectedHash && current.mode === entry.before.mode) rmSync(entry.path);
          else restored = false;
        }
        rmSync(entry.temporary, { force: true });
      } catch { restored = false; }
    }
    for (const directory of createdDirectories.reverse()) { try { rmdirSync(directory); } catch { /* Keep nonempty directories, including recovery backups. */ } }
    throw new Error(restored ? "来源迁移写入失败，已恢复原文件；请重新检查后重试" : "来源迁移写入失败且自动恢复未完成，请保留 Profile 中的备份文件并检查目录权限");
  }
  for (const entry of staged) if (entry.movedOriginal) { try { rmSync(entry.backup); } catch { /* A harmless backup is preferable to reverting a completed transaction. */ } }
