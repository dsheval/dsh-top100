...
export function readBundleProvenance(name: string, profile: string, profileDirectory?: string): InstallPreflight["provenance"] | null {
  ...
  if (!entry || entry.profile !== profile || entry.preflight.kind !== "bundle"
    || entry.preflight.provenance.packageName?.toLowerCase() !== name.toLowerCase()) return null;
...
export function renderBundleProvenance(config: PluginResolvedConfig, preflights: readonly InstallPreflight[]): string {
...
export function readSkillProvenance(name: string, profile: string, profileDirectory?: string): Pick<InstalledSkill, "digest" | "files"> | null {
  const ledger = readLedger(ledgerPath({ profile, profileDirectory, dataUrl: "" }));
  const entry = ledger.records[`skill:${name.toLowerCase()}`];
  if (!entry || entry.profile !== profile || entry.preflight.kind !== "skill") return null;
