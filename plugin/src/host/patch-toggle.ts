...
export function readUserPatchState(patchPath: string): PatchState {
  ...
  const forced: string[] = [];
  let text = "";
  try { text = readFileSync(patchPath, "utf8"); } catch { return { disables, forced }; }
...
export function parseInsertedIds(text: string): string[] {
  const ids: string[] = [];
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (id !== null && insertIndent !== null && indent > insertIndent && !ids.includes(id[1])) ids.push(id[1]);
  }
  return ids;
}
...
export function rowIdsForPackage(profile: string, packageName: string, explicitDir?: string): string[] {
  ...
  try {
    ...
    if (typeof manifest.dsh?.bundle?.patch === "string") candidates.unshift(manifest.dsh.bundle.patch);
  } catch { /* package missing */ }
  for (const relative of candidates) {
    try { for (const id of parseInsertedIds(readFileSync(join(packageDir, relative), "utf8"))) ids.add(id); } catch { /* no patch */ }
  }
  if (ids.size === 0) {
    const fallback = packageName.replace(/^@[^/]+\//, "").replace(/^@/, "");
    if (ROW_ID_RE.test(fallback)) ids.add(fallback);
  }
  return [...ids].filter((id) => ROW_ID_RE.test(id));
}

function escapeRegExp(value: string): string { return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); }
function rowBlock(rowId: string, disabled: boolean): string { return `- id: ${rowId}\n  disabled: ${disabled ? "true" : "false"}\n`; }

function withPlaceholderRestored(text: string): string {
  if (text.replace(/^[ \t]*#.*$/gm, "").trim() !== "") return text;
...
function appendPatchEntry(patchPath: string, block: string): { ok: boolean; reason: string | null } {
  let text = "";
  try { text = readFileSync(patchPath, "utf8"); } catch { writeFileSync(patchPath, block); return { ok: true, reason: null }; }
  ...
  if (/^[\[{]/.test(last)) return { ok: false, reason: "补丁层以顶层流式结构结尾，已拒绝写入" };
  writeFileSync(patchPath, `${text.endsWith("\n") ? text : `${text}\n`}${block}`);
  return { ok: true, reason: null };
}

export function setRowDisabled(patchPath: string, rowId: string, disabled: boolean): { ok: boolean; reason: string | null } {
  if (!ROW_ID_RE.test(rowId)) return { ok: false, reason: `无效的补丁行 id：${rowId}` };
  const state = readUserPatchState(patchPath);
  if (disabled && state.disables.includes(rowId)) return { ok: true, reason: null };
  if (!disabled && state.forced.includes(rowId) && !state.disables.includes(rowId)) return { ok: true, reason: null };
  ...
  if (disabled) {
    if (forceRe.test(text)) { writeFileSync(patchPath, text.replace(forceRe, rowBlock(rowId, true))); return { ok: true, reason: null }; }
    return appendPatchEntry(patchPath, rowBlock(rowId, true));
  }
  if (disableRe.test(text)) { writeFileSync(patchPath, withPlaceholderRestored(text.replace(disableRe, ""))); return { ok: true, reason: null }; }
...
export function removeRowBlocks(patchPath: string, rowIds: readonly string[]): void {
  if (!existsSync(patchPath) || rowIds.length === 0) return;
  ...
  if (text !== original) writeFileSync(patchPath, withPlaceholderRestored(text));
}
...
export function setPackageEnabled(profile: string, packageName: string, enabled: boolean, explicitDir?: string) {
  ...
  const rows = rowIdsForPackage(profile, packageName, explicitDir);
  if (rows.length === 0) return { ok: false, reason: "找不到可写入的补丁行", rows };
