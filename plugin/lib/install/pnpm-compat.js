...
function decodedOutput(output) {
    ...
    for (const line of output.split(/\r?\n/)) {
        ...
        try {
            ...
            if (typeof event.err?.message === "string")
                messages.push(event.err.message);
        }
        catch {
...
export async function withPnpmRecovery(run, profile, args, explicitDir) {
    ...
    else if (failure?.code === "release-age"
    ...
    else if (failure?.code === "host-peer"
        && failure.packageName
        && (command === "add" || command === "remove")
