...
function assertSourceDirectory(checkout, directory) {
    ...
    for (const part of ["", ...path.split(sep).filter(Boolean)]) {
        ...
        if (info.isSymbolicLink())
            throw new Error("Skill 来源目录包含符号链接，已拒绝安装");
        if (!info.isDirectory())
            throw new Error("Skill 来源不是目录");
    }
    const realPath = relative(realpathSync(checkout), realpathSync(directory));
    if (isAbsolute(realPath) || realPath === ".." || realPath.startsWith(`..${sep}`))
        throw new Error("Skill 来源超出仓库目录");
}
