...
function originalSelector(name, installedSpec, version, provenance) {
    if (!provenance || typeof provenance.packageName !== "string" || provenance.packageName.toLowerCase() !== name.toLowerCase()
        ...
        || !Number.isFinite(provenance.verifiedAt))
        return null;
    ...
    if (!resolved || !requested || resolved.name.toLowerCase() !== name.toLowerCase()
        ...
        || !sameExactVersion(version, resolved.selector))
        return null;
    return requested.selector ?? "latest";
}
export function resolveUpdateSource(name, installedSpec, options = {}) {
    const strategy = options.strategy ?? "preserve";
    if (strategy !== "preserve" && strategy !== "latest")
        throw new Error("更新策略无效");
    ...
    if (!installed)
        return null;
    if (strategy === "latest")
        return { target: `${name}@latest`, policy: "切换到 npm latest 频道" };
    const original = originalSelector(name, installedSpec, options.version ?? null, options.provenance);
