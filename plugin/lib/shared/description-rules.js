...
export function isDescriptionReviewDate(value) {
...
export function descriptionStatusFor(value) {
    if (value === undefined)
        return undefined;
    if (value && typeof value === 'object') {
        ...
        if (typeof status.state === 'string' && ['pending', 'review-required', 'missing-source', 'retry', 'stale'].includes(status.state)
            ...
            return { state: status.state, reason: cleanDescription(status.reason).slice(0, 200),
