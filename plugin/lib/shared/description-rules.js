...
export function matchesReviewedIdentity(entry, sourceInstall, sourceType) {
    if (sourceType !== undefined && sourceType !== (entry.type ?? null))
        return false;
...
export function matchesReviewedDescriptionSource(entry, review, context = {}, allowSelectedRootDescription = false) {
    if (!matchesReviewedIdentity(entry, review.sourceInstall, review.sourceType))
        return false;
    if (review.enforceSourceMatch && entry.install?.discovery?.status
        && entry.install.discovery.status !== 'verified')
        return false;
    if (review.sourceScope === 'verified-function')
        return Boolean(review.sourceInstall?.functionEvidence);
    const documentHash = entry.install?.discovery?.readme?.documentSha256;
    if (review.sourceDocumentHashes?.length && documentHash && !review.sourceDocumentHashes.includes(documentHash))
        return false;
    ...
    if (entry.readmeSummary === undefined && review.enforceSourceMatch
        && cleanDescription(entry.descriptionZh) === review.descriptionZh)
        return true;
    return [review, ...(review.sourceVariants ?? [])].some(source => (source.sourceDescription === (entry.description || '')
 ...
 */
export function descriptionQualityIssue(value) {
    const text = cleanDescription(value);
    if (!isChineseDescription(text) || isPlaceholder(text))
        return null;
    if (/(?:…+|\.{3,}|[:：])$/.test(text))
        return '简介是未完成的句子或列表引导语。';
    if (/^(?:衷心|特别)?感谢|^致谢[：:]/.test(text))
        return '简介是致谢，没有说明插件用途。';
    if (/^[·•\-\s]*(?:左|右|上|下)(?:图|侧)?[：:]/.test(text))
        return '简介是脱离上下文的图片说明。';
    if (/^.{1,60}(?:源自|得名于|取名自).{0,40}(?:神话|女神|之名)/.test(text))
        return '简介只解释名称来历。';
    if (/^(?:PATH\s*上|(?:本)?仓库已提交|前提[：:])|^需要\s*(?:Node|pnpm|npm|官方\s*dsh)/i.test(text))
        return '简介只说明安装或运行前提。';
    if (/^安装后.{0,30}(?:调用|使用)(?:本|该)?插件(?:注册|提供)的工具(?:即可)?[。！.]?$/.test(text))
        return '简介只有通用安装使用说明，没有说明工具用途。';
    if (/^纯\s*(?:Node(?:\.js)?|Python|JavaScript|TypeScript)\s*实现[，,、\s]*(?:无网络(?:依赖)?[，,、\s]*)?(?:无外部服务)?[。.!！]?$/i.test(text))
        return '简介只有实现语言或依赖说明。';
    if (/^.{1,100}(?:是|属于).{0,70}(?:基础插件|基础组件)[。.!！]?$/.test(text))
        return '简介只说明组件身份，没有说明功能。';
    if (/^(?:DeepSeek Harness|DSH).{0,20}(?:测试版|测试阶段|公开测试)/i.test(text))
        return '简介只说明宿主的测试状态。';
    if (/^配对\s*\d+\s*胜\s*\//.test(text))
        return '简介是测试成绩，没有说明插件用途。';
    if (/^(?:摘一段[，,]\s*生一枝|把每一分模型开销[，,]\s*看得清清楚楚)[。！]?$/.test(text))
        return '简介只有宣传口号，没有说明具体功能。';
    return null;
}
...
export function descriptionFor(entry, reviewed = {}, context = {}) {
    ...
    if (review?.reviewRequiredReason)
        return PENDING_DESCRIPTION_ZH;
    ...
    if (review && matchesReviewedDescriptionSource(entry, review, context)) {
        ...
        if (isUsableChineseDescription(chinese))
            return chinese;
    }
    if (review?.enforceSourceMatch)
        return PENDING_DESCRIPTION_ZH;
    ...
    if (chinese === PENDING_DESCRIPTION_ZH)
        return PENDING_DESCRIPTION_ZH;
    if (isUsableChineseDescription(chinese))
        return chinese;
    ...
    if (descriptionQualityIssue(chinese))
        return PENDING_DESCRIPTION_ZH;
    if (entry.install?.repositoryPath || entry.installRepositoryPath)
        return PENDING_DESCRIPTION_ZH;
