...
const PORTS = { "https:": "443", "http:": "80", "ssh:": "22", "git:": "9418" };
export function parseGitHubSource(value, purpose = "install") {
    if (typeof value !== "string")
        return null;
    let source = value.trim().replace(/^git\+/i, "");
    if (!source || source.length > 2048 || /[\\\s]/.test(source))
        return null;
    let repository;
    let selector = "";
    if (/^github:/i.test(source) || REPOSITORY.test(source.split("#")[0].replace(/\.git$/i, ""))) {
        ...
        if (!Object.hasOwn(PORTS, url.protocol) || url.hostname.toLowerCase() !== "github.com")
            return null;
        if ((url.port && url.port !== PORTS[url.protocol]) || url.search || url.password)
            return null;
        ...
        if (rawPath.split("/").some((part) => part === "." || part === ".." || /%/i.test(part)))
            return null;
        const match = /^\/([^/]+)\/([^/]+?)(?:\.git)?(\/.*)?$/i.exec(url.pathname);
        if (!match)
            return null;
        if (purpose === "install" && match[3] && match[3] !== "/")
            return null;
    ...
    if (purpose === "repository")
        return { repository: repository.toLowerCase(), ref: null, path: null };
    let ref = null;
    let path = null;
    for (const parameter of selector ? selector.split("&") : []) {
        if (parameter.startsWith("path:")) {
            ...
            if (ref !== null || !/^[A-Za-z0-9._~+/:=-]+$/.test(parameter))
                return null;
            ref = parameter;
        }
    }
...
export function githubRepositoryIdentity(value) {
    const url = value !== null && typeof value === "object" ? value.url : value;
    return parseGitHubSource(url, "repository")?.repository ?? null;
}
export function githubInstallTarget(source) {
