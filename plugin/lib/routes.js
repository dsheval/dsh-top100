...
import { cancelActive, progress, runDshPlugin } from "./dsh-cli.js";
import { queryOf, readJsonBody, sameOrigin, sendJson } from "./http.js";
import { FULL_NAME_RE, PROFILE_RE, isCordisEntry, resolveInstallSpec } from "./install-spec.js";
import { InstallVerificationError, verifyInstallSpec } from "./install-verify.js";
import { allowPackageBuild } from "./allow-builds.js";
...
function batchSnapshot(batchId) {
    ...
    if (!batch)
        return null;
    const batchJobs = batch.jobIds.map((jobId) => jobs.get(jobId)).filter(Boolean).map(publicJob);
    return {
        batchId,
        createdAt: batch.createdAt,
        jobs: batchJobs,
        completed: batchJobs.filter((job) => ["installed", "failed", "cancelled"].includes(job.phase)).length,
...
function updateJob(job, phase, patch = {}) {
    ...
    if (["installed", "failed", "cancelled"].includes(phase))
...
function cancelJob(jobId) {
    ...
    if (!job || ["installed", "failed", "cancelled"].includes(job.phase))
        return false;
        ...
        host.webServer.register({
            ...
            handler(_request, response) {
                sendJson(response, 200, {
                    ...
                    profile: config.profile,
                    progress,
                    activeJobs: [...jobs.values()].filter((job) => !["installed", "failed", "cancelled"].includes(job.phase)).map(publicJob),
                });
            },
        }),
        host.webServer.register({
