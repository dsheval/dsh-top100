...
import { isInstalledEntry, resolveInstallSpec } from "./install-spec.js";
...
export function filterCatalog(document, options) {
    ...
    const matched = source.filter((entry) => matchesQuery(entry, options.query)).map((entry) => annotate(entry, options.installed));
    return {
