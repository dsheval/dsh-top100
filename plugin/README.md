...
<p align="center">
  ...
  <a href="https://github.com/dsheval/dsh-top100/releases/tag/v1.3.6"><img alt="正式版本 v1.3.6" src="https://img.shields.io/badge/release-v1.3.6-2f6f68?style=flat-square"></a>
...
建议使用 **Node.js 24 LTS** 和 **DSH Web 0.1.5-rc.2**。本版保留已有旧宿主兼容范围，实际验证与限制见[兼容说明](https://github.com/dsheval/dsh-top100/blob/main/docs/release-1.3.5.md)。普通 npm/npx 用户请在 DSH 源码目录外，依次运行：

```sh
npx @deepseek-ai/dsh@0.1.5-rc.2 plugin --profile web add @dsheval/dsh-top100-plugin@1.3.6
...
minimumReleaseAgeExclude:
  - '@dsheval/dsh-top100-plugin@1.3.6'
```
...
```sh
npx @deepseek-ai/dsh@0.1.5-rc.2 plugin --profile web add -w @dsheval/dsh-top100-plugin@1.3.6
```
...
Top100 自身的版本、只读检查更新、榜单数据源及帮助集中在 **设置 → 插件 → 插件配置 → Top100 插件排行**。检查更新不会自动安装；榜单地址仅在点击保存后通过 DSH 设置服务写入，当前连接无写入权限时显示为只读。
...
```sh
pnpm dsh plugin --profile web add @dsheval/dsh-top100-plugin@1.3.6
```
...
```sh
dsh plugin --profile web add @dsheval/dsh-top100-plugin@1.3.6
dsh web
```
