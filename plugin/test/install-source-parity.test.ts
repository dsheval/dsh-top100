...
describe("collector, website and installed plugin use identical source rules", () => {
  it.each([
    ...
    ["dsh plugin add --save-exact @acme/demo@1.2.3", "@acme/demo@1.2.3"],
    ["dsh plugin --profile web add -w @acme/demo", "@acme/demo"],
    ["corepack pnpm dsh plugin --profile web add @acme/demo", "@acme/demo"],
    ["pnpm exec dsh plugin add npm:@acme/demo@latest", "@acme/demo@latest"],
    ["dsh plugin add @acme/demo --registry=https://registry.npmjs.org/", "@acme/demo"],
    ["dsh plugin add github:acme/demo#main\ndsh plugin add @acme/demo@1.2.3", "@acme/demo@1.2.3"],
    ["dsh plugin --profile tui add @acme/demo", null],
    ["corepack pnpm exec dsh plugin --profile research add @acme/demo", null],
    ["dsh plugin add @acme/demo --registry=https://mirror.example/", null],
