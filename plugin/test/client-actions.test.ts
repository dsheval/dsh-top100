...
async function managedSetup(jobResponse?: unknown) {
  const pending = deferred(); const entry = { name: "demo", kind: "bundle", descriptionZh: "演示", protected: false, local: false, updateAvailable: true, enabled: true, version: "1.0.0", activationState: "live" };
  vi.stubGlobal("fetch", vi.fn((url: string, init?: RequestInit) => { requests.push({ url, init }); if (url.includes("status")) return Promise.resolve(json({ activeBatches: [] })); if (url.includes("update-preflight")) return pending.promise; if (url.includes("install-jobs") && jobResponse) return Promise.resolve(json(jobResponse)); if (url === "/dsh-top100/manage") return Promise.resolve(json({ batchId: "batch", jobs: [], total: 2, completed: 0 })); return Promise.resolve(json({ items: [entry, { ...entry, name: "second" }], total: 2, profile: "web" })); }));
...
describe("actual managed update actions", () => {
  it("submits all pinned approvals only after reviewing and accepting the full batch", async () => {
    const { pending, tree } = await managedSetup(); button(tree, "updateAll").props.onClick(); expect(JSON.parse(requests.at(-1)!.init!.body as string)).toEqual({ names: ["demo", "second"] }); expect(requests.some((r) => r.url === "/dsh-top100/manage")).toBe(false);
  ...
  it("rejects a partial batch and retries the entire preflight", async () => {
    ...
    expect(text(current)).toContain("updatePreflightIncomplete"); expect(elements(current).some((e) => e.type === UpdateReview)).toBe(false); button(current, "retry").props.onClick(); expect(JSON.parse(requests.at(-1)!.init!.body as string)).toEqual({ names: ["demo", "second"] }); expect(requests.some((r) => r.url === "/dsh-top100/manage")).toBe(false);
  });
  it.each(["failed", "cancelled"])("shows %s recovery details and obtains new approval on retry", async (phase) => {
    ...
    button(current, "retry").props.onClick(); expect(requests.at(-1)!.url).toBe("/dsh-top100/update-preflight"); expect(JSON.parse(requests.at(-1)!.init!.body as string)).toEqual({ names: ["demo"] });
  ...
  it.each(["cancel", "unmount"])("discards a late full batch after %s", async (action) => {
    const { pending, tree } = await managedSetup(); button(tree, "updateAll").props.onClick(); const request = requests.at(-1)!; if (action === "cancel") button(render(ManagedPage), "cancel").props.onClick(); else host.reset();
