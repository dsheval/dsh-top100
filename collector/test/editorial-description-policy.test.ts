...
import reviewed from "../config/reviewed-descriptions.json";
import { descriptionFor, matchesReviewedReadme, descriptionQualityIssue, PENDING_DESCRIPTION_ZH } from "../src/description-rules.js";
function withReviewedDescription<T extends RankingEntry>(entry: T, context = {}): T {
  return { ...entry, descriptionZh: descriptionFor(entry, reviewed, context) };
}
import { filterCatalog } from "../../plugin/src/host/catalog.js";
import { recommendationResult } from "../../plugin/src/host/recommendations.js";
import type { RankingEntry, RankingsDocument } from "../../plugin/src/shared/types.js";
...
function published(entry: RankingEntry): RankingEntry { return { ...entry, descriptionPolicy: 'server-v1', descriptionZh: descriptionFor(entry, reviewed) }; }
function document(entry: RankingEntry, snapshotId?: string): RankingsDocument {
  return { schemaVersion: 2, snapshotId, generatedAt: "2026-09-04T06:02:14.458Z", snapshotDate: "2026-09-04",
    rankings: { total: [published(entry)], hot: [published(entry)], rising: [published(entry)] } };
}
...
describe("shared editorial descriptions", () => {
  ...
  it("uses an unchanged historical review for compact Agent recommendations", () => {
    ...
    expect(recommendationResult(compact, { query: fullName }).items[0].description).toBe(PENDING_DESCRIPTION_ZH);
    expect(entry.descriptionZh).toContain("资料不足");
  });
