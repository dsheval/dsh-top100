...
function fixture(missing = false, short = false) {
  ...
  const document = { schemaVersion: 2, generatedAt: '2026-09-16T00:00:00Z', snapshotDate: '2026-09-16',
    definitions: { total: 'total', hot: 'hot', rising: 'rising' }, categories: [], rankings: { total: rows, hot: short ? [] : rows, rising: short ? rows.slice(0, 2) : rows }, directories: { skills: [] } } as unknown as RankingsDocument;
...
describe('publication acceptance', () => {
  it('accepts honest empty and short boards independently of the full directory size', async () => {
    const { dir } = fixture(false, true);
    const report = await auditPublication({ publicDirectory: dir, expectedDate: '2026-09-16' });
    expect(report.pluginCount).toBe(6);
    expect(report.boards.hot.total).toBe(0);
    expect(report.boards.rising.total).toBe(2);
  });
